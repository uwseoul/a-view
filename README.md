...
- **SleepGuard 디버그 오버레이** — 수동 토글, 카운트다운 타이머, 상세 상태 표시
- **Port Killer 추가** — 포트 스캔, 강제 종료, 실시간 검색, 분류 필터(Web/DB/Dev/Sys/기타) 지원
- **한/영 언어 전환** — 전체 UI 텍스트 한국어/영어 전환 지원
- **UI 리파인** — 상단 탭을 헤더에 통합하고, 상대시간 표기와 밀도 조정으로 정보 가독성 개선
- **성능 최적화** — 변경 감지 기반 렌더 스킵, 포트 검색 debounce, 적응형 폴
링, 비활성 탭 포트 스캔 최소화 적용
- **에이전트 상태 정교화** — Running/Stalled/Completed/Failed 분류 및 요약 카운팅 로직 개선
- **프로세스 종료 개선** — 한글 깨짐 현상 수정 및 종료 실패 케이스 대응
- **번들 배포** — macOS `.dmg`, Windows `.exe` 설치 파일 기준으로 변경
...
- **macOS**: `.dmg` 다운로드 후 드래그 앤 드롭으로 설치
- **Windows**: `.exe` 인스톨러 실행
...
macOS에서 처음 실행할 때 아래 팝업가 뜨면 **Gatekeeper/격리 속성** 문제입니다.

> 'a-view.app'은(는) 손상되었기 때문에 열 수 없습니다. 해당 항목을 휴지통으로 이동해야 합니다.

**해결 방법:**

1. **터미널**을 열고 아래 명령어를 실행합니다. (`/Applications/a-view.app` 경로를 실제 설치 경로로 바꾸세요.)
   ```bash
   xattr -rd com.apple.quarantine /Applications/a-view.app
   ```
2. 앱을 다시 실행합니다.

> 참고: 이 메시지는 파일이 실제로 손상된 것이 아니라, 다운로드된 앱의 보안 검증(코드 서명/공증)이 macOS 기본 정책에 의해 차단된 것입니다.
